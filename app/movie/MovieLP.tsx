      ...
      <section className="faq" id="faq">
        <div className="container">
          ...
          <div className="faq-list">
            {[
              { q: "ここは動画編集のスクールですか？", a: "いいえ、当スタジオは「スクール」ではなく企業案件を手がける「制作スタジオ」です。携わった案件がそのままあなたの実績として積み上がります。なお、パッソアニメーションスタジオは働くことに障がいのある方のための就労継続支援B型事業所です。" },
              { q: "就労継続支援B型とは何ですか？", a: "雇用契約を結ばずに、仕事に携わることができる福祉サービスのひとつです。\n\n当スタジオでは、企業から受託した映像制作に携わりながら工賃を受け取ることができます。手がけた仕事はそのままあなたのポートフォリオになります。\n\n無償または低負担でご利用いただけます。" },
              ...
              { q: "在宅での制作（リモートワーク）は可能ですか？", a: "当スタジオでは、原則としてスタジオへお越しいただいての制作をお願いしております。ディレクターやチームのメンバーと同じ空間でコミュニケーションを取りながら進めることが、企業案件のクオリティを高め、チームとの連携も深まると考えているからです。" },
              { q: "利用するための条件はありますか？", a: "精神・知的・身体障がい・難病があり、原則として「障がい者手帳」をお持ちの方、あるいは医師の診断や意見書がある方にご利用いただけます。なお、ご利用にあたっては「受給者証」という証明書の取得が必要です（詳しくは下記をご覧ください）。" },
              { q: "「受給者証」とは何ですか？持っていなくても利用できますか？", a: "受給者証とは、障害福祉サービスを利用するために必要な証明書で、お住まいの市区町村の窓口で申請します。障がい者手帳をお持ちでない方でも、主治医の意見書（または診断書）があれば申請できる場合があります。ただし申請には市区町村による審査があり、状況によって異なります。「自分は手続きできるのだろうか」「手続きが不安」という方も、ディレクターが一緒に確認しますので、LINEよりご連絡ください。" },
              { q: "制作に対する報酬は支払われますか？", a: "はい。作業時間に応じた基本工賃に加え、企業案件に携わった成果や貢献度に応じた『生産活動ボーナス』を上乗せしてお支払いしています。頑張りがしっかり評価される仕組みです。" },
              { q: "この先どんな将来に繋がっていきますか？", a: "ここで制作するメンバーが描く将来は本当にさまざまです。「フリーランスとして自分で案件を受けたい」という方もいれば、「動画編集で月に数万円の収入を得たい」という方、「企業のSNS担当として就職したい」という方もいます。ただどの道に進むにしても「ポートフォリオ（制作実績）」は必ず見られます。数も種類も多いほど有利です。多彩な企業案件に携わり、制作を重ねる中で自然とポートフォリオが充実していきます。" },
            ].map((faq, i) => (
